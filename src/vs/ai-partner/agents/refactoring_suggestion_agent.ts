...
export class RefactoringSuggestionAgent extends Agent {
     ...
     */
    private async handleGetRefactoringSuggestions(message: A2AMessage): Promise<void> {
        ...
        try {
            ...
            const suggestions = await llmService.getCompletion(prompt);
            ...
            await this.sendErrorResponse(message, `Failed to get refactoring suggestions: ${error}`);
        }
    }
