...
function getExamples(): string {
    return `### ACTION EXAMPLES (Brainstorm Flow)
...
1. **Observation**:
   - [Result Analysis]: Found new 'Vapor Mode' data.
   - [Unexpected]: Vue is claiming significantly higher performance than React Fiber.
2. **Verification (Lightweight)**:
   - [Knowledge Gap]: **Check Required**. Is React Compiler (Forget) comparable?
   - [Complexity]: **+10**. Need to compare Vapor vs React Compiler.
3. **Refinement**:
   - [Adjustment]: Search for React Compiler benchmarks too.
4. **Plan**: [Next Action: web_search("React Compiler benchmark")].
</thinking>
> Tool: web_search(...)
}`;
}
