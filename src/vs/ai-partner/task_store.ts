...
export class TaskStore {
    ...
    public static getInstance(context: vscode.ExtensionContext): TaskStore {
    ...
    public async updateTaskArtifacts(id: string, artifact: Artifact, append: boolean): Promise<Task | undefined> {
        const task = this.tasks.get(id);
        if (task) {
            // Normalize artifacts to a concrete array for safe operations
            const artifacts: any[] = ((task as any).artifacts ?? []) as any[];
            const existingArtifactIndex = artifacts.findIndex((a: any) => a.artifactId === (artifact as any).artifactId);
            if (existingArtifactIndex > -1) {
                if (append) {
                    artifacts[existingArtifactIndex].parts = [
                        ...(artifacts[existingArtifactIndex].parts || []),
                        ...((artifact as any).parts || [])
                    ];
                } else {
                    artifacts[existingArtifactIndex] = artifact as any;
                }
            } else {
                artifacts.push(artifact as any);
            }
            (task as any).artifacts = artifacts as any;
            this.tasks.set(id, task);
            await this.persistTasks();
            return task;
        }
